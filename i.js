 ...
 * Use this [2008 - Javascript Gettext](https://sourceforge.net/projects/jsgettext.berlios/?source=navbar)
            ...
            if (domain.type == 'json') {
                ...
                if (! this.try_load_lang_json(link.href) ) {
                    ...
                    switch (this.locale_format) {
                        ...
                        case 2: //locale name format is something like *en_US.utf8*. will try to use *en_US* or *en* format
                            ...
                            link.href = domain.path+"/"+this.locale.split('.')[0]+"/LC_MESSAGES/"+domain.value+".json" ;
                            ...
                            if (! this.try_load_lang_json( link.href )) {
                                ...
                                link.href = domain.path+"/"+this.locale.split('_')[0]+"/LC_MESSAGES/"+domain.value+".json" ;
                                
                                if (! this.try_load_lang_json( link.href )) {
                                    
                                    link.href = domain.path+"/"+this.locale+"/LC_MESSAGES/"+domain.value+".json";
                                    console.warn("iJS-gettext:'try_load_lang_json': failed. Unable to exec XMLHttpRequest for link ["+link.href+"]") ;
                                }
                            }
                            break;
                        case 1://locale name format is something like *en_US*. will try to use *en* format
                            ...
                            if (! this.try_load_lang_json( link.href )) {
                                ...
                                link.href = domain.path+"/"+this.locale+"/LC_MESSAGES/"+domain.value+".json";
                                console.error("iJS-gettext:'try_load_lang_json': failed. Unable to exec XMLHttpRequest for link ["+link.href+"]") ;
                            }
                            break;
                ...
                if (! this.try_load_lang_po(link.href) ) {
                    ...
                    switch (this.locale_format) {
                        ...
                        case 2: //locale name format is something like *en_US.utf8*. will try to use *en_US* or *en* format
                            console.warn("iJS-gettext:'try_load_lang': domaine: *"+domain.value+"* not found with locale: *"+this.locale+"* format. Will try to use *"+this.locale.split('.')[0]+"* format...") ;
                            link.href = domain.path+"/"+this.locale.split('.')[0]+"/LC_MESSAGES/"+domain.value+".po" ;

                            if (! this.try_load_lang_po( link.href )) {

                                console.warn("iJS-gettext:'try_load_lang': domaine: *"+domain.value+"* not found with locale: *"+this.locale.split('.')[0]+"* format. Will try to use *"+this.locale.split('_')[0]+"* format...") ;
                                link.href = domain.path+"/"+this.locale.split('_')[0]+"/LC_MESSAGES/"+domain.value+".po" ;

                                if (! this.try_load_lang_po( link.href )) {

                                    link.href = domain.path+"/"+this.locale+"/LC_MESSAGES/"+domain.value+".po";
                                    console.warn("iJS-gettext:'try_load_lang_po' failed. Unable to exec XMLHttpRequest for link ["+link.href+"]") ;
                                }
                            }
                            break;
                        case 1://locale name format is something like *en_US*. will try to use *en* format
                            ...
                            if (! this.try_load_lang_po( link.href )) {
                                ...
                                link.href = domain.path+"/"+this.locale+"/LC_MESSAGES/"+domain.value+".po";
                                console.error("iJS-gettext:'try_load_lang_po' failed. Unable to exec XMLHttpRequest for link ["+link.href+"]") ;
                            }
                            break;
